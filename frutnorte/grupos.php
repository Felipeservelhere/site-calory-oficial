...
$ordem_campo = isset($_GET['ordem']) ? $_GET['ordem'] : 'nome';
...
$campos_permitidos = ['id', 'codgrupo', 'nome', 'perc_mb', 'perc_avista', 'total_produtos'];
if (!in_array($ordem_campo, $campos_permitidos)) $ordem_campo = 'nome';
if (!in_array($ordem_direcao, ['ASC', 'DESC'])) $ordem_direcao = 'ASC';

// ==================== CONSTRUÇÃO DA QUERY ====================
$where_conditions = ["g.idcliente = ?"]; // Filtrar pelo cliente/empresa
$params = [$empresa_id];

if (!empty($busca)) {
    $where_conditions[] = "g.nome LIKE ?";
    $params[] = "%$busca%";
}
...
// ==================== QUERY PRINCIPAL ====================
$sql = "SELECT g.id, g.idcliente, g.codgrupo, g.nome, g.perc_mb, g.perc_avista,
               COUNT(p.id) as total_produtos
        FROM grupos g
        LEFT JOIN produtos p ON g.codgrupo = p.codgrupo AND p.idcliente = g.idcliente
        $where_clause
        GROUP BY g.id, g.idcliente, g.codgrupo, g.nome, g.perc_mb, g.perc_avista
...
$sql_count = "SELECT COUNT(DISTINCT g.id) as total FROM grupos g $where_clause";
        ...
        <div class="page-header">
            ...
            <div class="header-content">
                <div class="title-section">
                    <h1 class="page-title">
                        ...
                        <div class="title-content">
                            <span class="title-main">Gestão de Grupos</span>
                            <p class="title-subtitle"><?= $total_registros ?> grupo(s) encontrado(s)</p>
                        </div>
                    </h1>
                </div>
                <div class="header-actions">
                    ...
                    <a href="acoes_grupos/cadastro-grupo.php" class="btn btn-primary">
        ...
        <div class="groups-container">
            <?php if (empty($grupos)): ?>
                <div class="empty-state">
                    ...
                    <p class="empty-text">Não há grupos cadastrados com os filtros selecionados.</p>
                    <a href="acoes_grupos/cadastro-grupo.php" class="btn btn-primary">
                        <i class="fas fa-plus"></i>
                        ...
                        <table>
                            <thead>
                                <tr>
                                    ...
                                    <th class="sortable" onclick="ordenarPor('perc_mb')">
                                        % Margem Bruta
                                        <?php if ($ordem_campo === 'perc_mb'): ?>
                                            <i class="fas fa-sort-<?= $ordem_direcao === 'ASC' ? 'up' : 'down' ?>"></i>
                                        <?php else: ?>
                                            <i class="fas fa-sort"></i>
                                        <?php endif; ?>
                                    </th>
                                    <th class="sortable" onclick="ordenarPor('perc_avista')">
                                        % À Vista
                                        <?php if ($ordem_campo === 'perc_avista'): ?>
                                            <i class="fas fa-sort-<?= $ordem_direcao === 'ASC' ? 'up' : 'down' ?>"></i>
                                        <?php else: ?>
                                            <i class="fas fa-sort"></i>
                                        <?php endif; ?>
                                    </th>
                                    <th class="sortable" onclick="ordenarPor('total_produtos')">
                                        Produtos
                                        <?php if ($ordem_campo === 'total_produtos'): ?>
                                            <i class="fas fa-sort-<?= $ordem_direcao === 'ASC' ? 'up' : 'down' ?>"></i>
                                        <?php else: ?>
                                            <i class="fas fa-sort"></i>
                                        <?php endif; ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="percentage">
                                                <?= $grupo['perc_mb'] !== null ? number_format($grupo['perc_mb'], 2, ',', '.') . '%' : '<span class="text-muted">-</span>' ?>
                                            </span>
                                        </td>
                                        <td>
                                            <span class="percentage">
                                                <?= $grupo['perc_avista'] !== null ? number_format($grupo['perc_avista'], 2, ',', '.') . '%' : '<span class="text-muted">-</span>' ?>
                                            </span>
                                        </td>
                                        <td>
                                            <span class="product-count">
                                                <i class="fas fa-box"></i>
                                                <?= $grupo['total_produtos'] ?> produto(s)
                                            </span>
                                        </td>
                                        <td>
                                            <div class="actions">
                                                <a href="acoes_grupos/visualizar-grupo.php?id=<?= $grupo['id'] ?>" class="btn-action btn-view" title="Visualizar">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <a href="acoes_grupos/editar-grupo.php?id=<?= $grupo['id'] ?>&codgrupo=<?= $grupo['codgrupo'] ?>" class="btn-action btn-edit" title="Editar">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <button onclick="confirmarExclusao(<?= $grupo['id'] ?>, '<?= htmlspecialchars(addslashes($grupo['nome'])) ?>', <?= $grupo['total_produtos'] ?>)" 
                        ...
                        <div class="pagination-container">
                            <div class="pagination">
                                <?php if ($pagina_atual > 1): ?>
                                    <a href="?pagina=<?= $pagina_atual - 1 ?>&busca=<?= urlencode($busca) ?>&ordem=<?= $ordem_campo ?>&direcao=<?= $ordem_direcao ?>" class="pagination-btn">
                                ...
                                <div class="pagination-numbers">
                                    <?php for ($i = max(1, $pagina_atual - 2); $i <= min($total_paginas, $pagina_atual + 2); $i++): ?>
                                        ...
                                        <?php else: ?>
                                            <a href="?pagina=<?= $i ?>&busca=<?= urlencode($busca) ?>&ordem=<?= $ordem_campo ?>&direcao=<?= $ordem_direcao ?>" class="pagination-number"><?= $i ?></a>
                                        <?php endif; ?>
                                    <?php endfor; ?>
                                </div>
...
<script>
    function confirmarExclusao(id, nome, totalProdutos) {
        let mensagem = `Tem certeza que deseja excluir o grupo "${nome}"?`;
        
        if (totalProdutos > 0) {
            mensagem += `\n\nATENÇÃO: Este grupo possui ${totalProdutos} produto(s) associado(s). Eles ficarão sem grupo após a exclusão.`;
        }
    ...
    function excluirGrupo(id) {
        ...
        loadingToast.innerHTML = `
            ...
            <span>Excluindo grupo...</span>
        `;
        toastContainer.appendChild(loadingToast);
        
        fetch('acoes_grupos/excluir_grupo.php', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                showToast('Erro ao excluir grupo: ' + data.message, 'error');
            }
        })
        .catch(error => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (mensagem === 'grupo_salvo') {
            showToast('Grupo cadastrado com sucesso!', 'success');
        } else if (mensagem === 'grupo_atualizado') {
            showToast('Grupo atualizado com sucesso!', 'success');
        } else if (mensagem === 'selecione_grupo') {
            showToast('Por favor, selecione um grupo para editar clicando no botão "Editar" na lista abaixo.', 'info');
        } else if (erro === 'grupo_nao_encontrado') {
            showToast('Grupo não encontrado. Verifique se o grupo ainda existe.', 'error');
        ...
        if (termoBusca) {
            ...
            const elementos = document.querySelectorAll('.group-name strong');
