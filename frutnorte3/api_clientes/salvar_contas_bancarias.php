...
try {
    ...
    // 👈 Log adicional: Registrar o input completo recebido para debug
    error_log("Input recebido completo: " . json_encode($input));
...
function handlePost($pdo, $input, $idcliente_empresa) {
    ...
    // 👈 Log adicional: Registrar action e codcliente extraídos
    error_log("POST Debug: action='$action', codcliente input='$input_codcliente', idcliente_empresa=$idcliente_empresa");
    
    // 👈 Se action não for definida, logar e lançar erro específico
    if (empty($action)) {
        error_log("POST Erro: Action não definida no input");
        throw new Exception('Ação (action) não especificada no request.');
    }
    
    // 👈 Validação de codcliente: Só obrigatória para ações que precisam (ex.: excluir). Para salvar temp e limpar, não precisa.
    switch ($action) {
        case 'salvar':
            error_log("POST Processando 'salvar': Sem necessidade de codcliente");
            // Para salvar temp, não precisa codcliente (é sessão do usuário logado na empresa)
            salvarContaTemp($input);
            break;
            
        case 'excluir':
            error_log("POST Processando 'excluir': Verificando codcliente");
            if (empty($input_codcliente)) {
                error_log("POST Erro em 'excluir': codcliente vazio");
        ...
        case 'limpar':
            error_log("POST Processando 'limpar': Sem necessidade de codcliente");
            // Para limpar sessão, não precisa codcliente
            limparSessao();
            break;
            
        default:
            error_log("POST Erro: Ação '$action' não reconhecida");
            throw new Exception('Ação não especificada ou inválida: ' . $action);
    }
}
